    ...
    public Optional<OrderBean> createOrder(OrderDTO orderDTO) {
     ...
     // --- 處理 Promotion 關聯 ---
        ...
        if (savedOrder.getStore() != null && savedOrder.getStore().getId() != null && savedOrder.getId() != null) {
            System.out.println("準備發送 WebSocket 通知給 StoreID: " + savedOrder.getStore().getId() + ", OrderID: " + savedOrder.getId());
...
//        }
     // 檢查日誌 1: 確認是否進入通知發送邏輯
        if (existingOrderBean.getStore() != null && existingOrderBean.getStore().getId() != null && existingOrderBean.getId() != null) {
            System.out.println("準備發送 WebSocket 通知給 StoreID: " + existingOrderBean.getStore().getId() + ", OrderID: " + existingOrderBean.getId());
