...
onMounted(async () => {
    ...
    console.log('[onMounted] localStorage ownerId:', ownerId)
    if (!ownerId) {
        alert('未取得業者ID，請重新登入')
        return
    }
    try {
        ...
        console.log('[onMounted] axios.get /api/owner result:', res)
        const data = res.data.owner || res.data
        console.log('[onMounted] owner data:', data)
        ...
        // debug: 初始同步值
        console.log('[onMounted] initial:', JSON.stringify(initial, null, 2))
        // debug: Pinia 狀態
        console.log('[onMounted] userStore:', JSON.stringify({
            ownerFullName: userStore.ownerFullName,
            ownerEmail: userStore.ownerEmail,
            phone: userStore.phone,
            ownerId: userStore.ownerId
        }, null, 2))
    } catch (e) {
        alert('取得帳戶資料失敗')
