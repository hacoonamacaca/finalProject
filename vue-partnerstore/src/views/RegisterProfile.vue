    ...
    // 密碼規則
    const minLen = 8
    const hasLen = computed(() => password.value.length >= minLen)
    ...
    <div class="profile-bg">
        ...
        <form @submit.prevent="onSubmit">
            ...
            <div class="col">
                <input type="text" v-model="lastName" class="form-control custom-input" placeholder="姓" required>
            </div>
            <div class="col">
                <input type="text" v-model="firstName" class="form-control custom-input" placeholder="名" required>
            </div>
            </div>
            ...
            >
                ...
                <!-- 關眼圖示 -->
                <path d="M17.94 17.94C16.14 19.25 14.12 20 12 20c-7 0-11-8-11-8a19.89 19.89 0 0 1 5.06-5.94M10.11 6.11A7.01 7.01 0 0 1 12 6c7 0 11 8 11 8a19.89 19.89 0 0 1-4.13 5.11M1 1l22 22" stroke="#666" stroke-width="2" stroke-linecap="round"/>
                <circle cx="12" cy="12" r="3" stroke="#666" stroke-width="2"/>
                </svg>
            </button>
            </div>
            ...
            <span :style="{color: strength.color, 'font-weight':'bold', 'margin-left': '6px', 'font-size':'15px'}">
                {{ strength.label }}
            </span>
            </div>
            ...
            <ul class="mb-1" style="list-style: none; padding-left: 0;">
                ...
                <span :class="{'check': hasLen}">
                    <svg v-if="hasLen" width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M6 12l4 4 8-8" stroke="#4caf50" stroke-width="2" stroke-linecap="round"/></svg>
                    <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="12" r="1.5" fill="#bbb"/><circle cx="15" cy="12" r="1.5" fill="#bbb"/></svg>
                </span>
                至少需要8個字
                </li>
                <li class="require-item">
                <span :class="{'check': hasLower}">
                    <svg v-if="hasLower" width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M6 12l4 4 8-8" stroke="#4caf50" stroke-width="2" stroke-linecap="round"/></svg>
                    <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="12" r="1.5" fill="#bbb"/><circle cx="15" cy="12" r="1.5" fill="#bbb"/></svg>
                </span>
                至少需要一個小寫字母 (a-z)
                </li>
                ...
                <span :class="{'check': hasNumber}">
                    <svg v-if="hasNumber" width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M6 12l4 4 8-8" stroke="#4caf50" stroke-width="2" stroke-linecap="round"/></svg>
                    <svg v-else width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="9" cy="12" r="1.5" fill="#bbb"/><circle cx="15" cy="12" r="1.5" fill="#bbb"/></svg>
                </span>
                至少需要一個數字 (0-9)
                </li>
